...
这个`Sampling`层接收两个输入：`mean (μ)` 和 `log_var (γ)`。使用函数`K.random_normal()`根据正态分布随机采样向量（形状为γ）平均值为 0 标准差为 1。然后乘以 exp(γ / 2)（这个值等于σ），最后加上μ并返回结果。这样就能从平均值为 0 标准差为 1 的正态分布采样编码向量。
