...
这个模型仅仅是输入量`GDP_per_capita`的线性函数，![\theta _{0}](img/tex-3d7e996adfe310516ca31c796df1ce8c.gif) 和 ![\theta _{1}](img/tex-92c9134527161fd7453fe848b821d8c7.gif) 是这个模型的参数，线性模型更一般化的描述指通过计算输入变量的加权和，并加上一个常数偏置项（截距项）来得到一个预测值。如公式 4-1：

公式 4-1：线性回归预测模型

![\hat{y} = \theta _{0} + \theta _{1}x _{1}+\theta _{2}x _{2}+\dots+\theta _{n}x _{n}](img/tex-f99876b625a13a0aad9631f61d934a61.gif)

+ ![\hat{y}](img/tex-5d28a7ba1a44a73b8c2ed21321697c59.gif) 表示预测结果
+ ![n](img/tex-7b8b965ad4bca0e41ab51de7b31363a1.gif) 表示特征的个数
+ ![x_{i}](img/tex-05e42209d67fe1eb15a055e9d3b3770e.gif) 表示第`i`个特征的值
+ ![\theta_{j}](img/tex-4c5645510c6b63f6bd0c770fceea4ac1.gif) 表示第`j`个参数（包括偏置项 ![\theta _{0}](img/tex-3d7e996adfe310516ca31c796df1ce8c.gif) 和特征权重值 ![\theta _{1},\theta _{2},\dots,\theta _{n}](img/tex-54552e28fe2d86eb74de3db6abb5aab8.gif)）
...
+ ![\theta](img/tex-2554a2bb846cffd697389e5dc8912759.gif) 表示模型的参数向量包括偏置项 ![\theta _{0}](img/tex-3d7e996adfe310516ca31c796df1ce8c.gif) 和特征权重值 ![\theta _{1}](img/tex-92c9134527161fd7453fe848b821d8c7.gif) 到 ![\theta _{n}](img/tex-f5c4858c5101df83fa23d1f57981c12b.gif)
+ ![\theta^T](img/tex-616944242afb697770bd2354c57e4773.gif) 表示向量![\theta](img/tex-2554a2bb846cffd697389e5dc8912759.gif)的转置（行向量变为了列向量）
+ ![\mathbf{x}](img/tex-70e59a996bd69a0c21878b4093375e92.gif) 为每个样本中特征值的向量形式，包括 ![x _{1}](img/tex-f1dd81ad21180e074e52e8cccdcbb172.gif) 到 ![x_{n}](img/tex-67b68721103b5a16194f4b3e3ec222db.gif)，而且 ![x_0](img/tex-3e0d691f3a530e6c7e079636f20c111b.gif) 恒为 1
+ ![\theta^T  \cdot \mathbf{x}](img/tex-dc285d84f74bea2336104ee5eafff150.gif) 表示 ![\theta^T](img/tex-616944242afb697770bd2354c57e4773.gif) 和![  \mathbf{x}](img/tex-7186298c04fa42047e7992afaf52dfe0.gif) 的点积
+ ![h_{\theta}](img/tex-b7ae93cd51ba1d38fe4ebac15eab3aa1.gif) 表示参数为 ![\theta](img/tex-2554a2bb846cffd697389e5dc8912759.gif) 的假设函数
...
在训练集 ![\mathbf{X}](img/tex-ca340abf4b48dc6d816137fbadf58b53.gif) 上使用公式 4-3 来计算线性回归假设 ![h_{\theta}](img/tex-b7ae93cd51ba1d38fe4ebac15eab3aa1.gif) 的均方差（![MSE](img/tex-a9fc1a03386ae38b64e06c8172994963.gif)）。
...
现在我们能够使用 ![\hat{\theta}](img/tex-0678caa04da34220a4e8dc041488b618.gif) 来进行预测：
...
正规方程需要计算矩阵 ![{\mathbf{X}}^T\cdot\mathbf{X}](img/tex-e5949d4f83eb4e13761f2b76ab62386e.gif) 的逆，它是一个 ![n * n](img/tex-6679fc33e499a90a99b97201f4d00ed5.gif) 的矩阵（![n](img/tex-7b8b965ad4bca0e41ab51de7b31363a1.gif) 是特征的个数）。这样一个矩阵求逆的运算复杂度大约在 ![O(n^{2.4})](img/tex-31ba2ac7dbc438cef695358d6e49deb3.gif) 到 ![O(n^3)](img/tex-4a7d22b39e93fbbcbe107e7a19e8bd34.gif) 之间，具体值取决于计算方式。换句话说，如果你将你的特征个数翻倍的话，其计算时间大概会变为原来的 5.3（![2^{2.4}](img/tex-61f8abcb13be8d0a51b2868de491d3a8.gif)）到 8（![2^3](img/tex-5b9a77af89d04a685b4f649da485aed3.gif)）倍。
...
假设浓雾下，你迷失在了大山中，你只能感受到自己脚下的坡度。为了最快到达山底，一个最好的方法就是沿着坡度最陡的地方下山。这其实就是梯度下降所做的：它计算误差函数关于参数向量![\theta](img/tex-2554a2bb846cffd697389e5dc8912759.gif)的局部梯度，同时它沿着梯度下降的方向进行下一次迭代。当梯度值为零的时候，就达到了误差函数最小值 。

具体来说，开始时，需要选定一个随机的![\theta](img/tex-2554a2bb846cffd697389e5dc8912759.gif)（这个值称为随机初始值），然后逐渐去改进它，每一次变化一小步，每一步都试着降低损失函数（例如：均方差损失函数），直到算法收敛到一个最小值（如图：4-3）。
...
事实上，损失函数的图像呈现碗状，但是不同特征的取值范围相差较大的时，这个碗可能是细长的。图 4-7 展示了梯度下降在不同训练集上的表现。在左图中，特征 1 和特征 2 有着相同的数值尺度。在右图中，特征 1 比特征 2 的取值要小的多，由于特征 1 较小，因此损失函数改变时，![\theta_1](img/tex-7672d625e9a2492987c50d3b87c04349.gif) 会有较大的变化，于是这个图像会在![\theta_1](img/tex-7672d625e9a2492987c50d3b87c04349.gif)轴方向变得细长。
...
使用梯度下降的过程中，你需要计算每一个 ![\theta_j](img/tex-cb0e17d96e58d55d1eb06dc1b14b7a7b.gif) 下损失函数的梯度。换句话说，你需要计算当![\theta_j](img/tex-cb0e17d96e58d55d1eb06dc1b14b7a7b.gif)变化一点点时，损失函数改变了多少。这称为偏导数，它就像当你面对东方的时候问："我脚下的坡度是多少？"。然后面向北方的时候问同样的问题（如果你能想象一个超过三维的宇宙，可以对所有的方向都这样做）。公式 4-5 计算关于 ![\theta_j](img/tex-cb0e17d96e58d55d1eb06dc1b14b7a7b.gif) 的损失函数的偏导数，记为 ![\frac{\partial }{\partial \theta_j}MSE(\theta)](img/tex-5c3b6b00cffc9732138715003b0e557a.gif)。
...
让我比较一下目前我们已经探讨过的对线性回归的梯度下降算法。如表 4-1 所示，其中 ![m](img/tex-6f8f57715090da2632453988d9a1501b.gif) 表示训练样本的个数，![n](img/tex-7b8b965ad4bca0e41ab51de7b31363a1.gif) 表示特征的个数。
...
`X_poly`现在包含原始特征![X](img/tex-02129bb861061d1a052c592e2dc6b383.gif)并加上了这个特征的平方 ![X^2](img/tex-75299c2520ca389119694b3da7cc7a84.gif)。现在你可以在这个扩展训练集上使用`LinearRegression`模型进行拟合，如图 4-13：
...
> 提示
>
> `PolynomialFeatures(degree=d)`把一个包含 ![n](img/tex-7b8b965ad4bca0e41ab51de7b31363a1.gif) 个特征的数组转换为一个包含 ![\frac{(n+d)!}{d!n!}](img/tex-bbbca49c7d6afb0938a5c37bb5b5fbcf.gif) 特征的数组，![n!](img/tex-388f554901ba5d77339eec8b26beebea.gif) 表示 ![n](img/tex-7b8b965ad4bca0e41ab51de7b31363a1.gif) 的阶乘，等于 ![1 * 2 * 3 \cdots * n](img/tex-62b4db05802cc87cf8ed00845ce751af.gif)。小心大量特征的组合爆炸！
...
可以看出，当只有两个类（![K=2](img/tex-5869b95a3404e737433d626520200848.gif)）时，此损失函数等同于 Logistic 回归的损失函数（对数损失；请参阅公式 4-17）。
